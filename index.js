...
app.post("/post-secret", async (req, res) => {
  ...
  try {
    const response = await axios.post(API_URL + "/secrets", {
      ...
      score: result.score // Assuming you meant to post `score` with the value `result.score`
    },config);
...
app.post("/put-secret", async (req, res) => {
  ...
  const searchId = req.body.id;
 try {
  const results = await axios.put(API_URL + "/secrets/" + searchId,{
    secret: result.secret,
    score: result.score
  },config)
  res.render("index.ejs", { content: JSON.stringify(results.data) });
  console.log("Love You ❤️")
 } catch (error) {
  res.render("index.ejs", { content: JSON.stringify(error.response.data) });
 }
